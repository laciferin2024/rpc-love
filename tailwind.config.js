...
export default {
  ...
  theme: {
  	extend: {
		...
		spacing: {
			// Semantic spacing scale (PRD: 2, 4, 6, 8, 12, 16, 24, 32)
			// Note: Tailwind defaults are preserved, these are additional semantic tokens
			'18': '4.5rem',
			'72': '18rem',
			'84': '21rem',
			'96': '24rem',
		...
		colors: {
			...
			warning: {
				...
				on: 'hsl(var(--warning-on))'
			},
			danger: {
				DEFAULT: 'hsl(var(--danger))',
				on: 'hsl(var(--danger-on))'
			},
			'focus-ring': 'hsl(var(--focus-ring) / var(--focus-ring-opacity))',
			selection: 'hsl(var(--selection) / var(--selection-opacity))',
			hover: 'hsl(var(--hover) / var(--hover-opacity))',
			active: 'hsl(var(--active) / var(--active-opacity))',
